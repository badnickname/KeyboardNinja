bool SceneTableLead::init(sf::RenderWindow &app) {
  ...
  if (NUMBERS <= 8)
    background_top_sprite.setTextureRect(
        sf::IntRect(0, 0, 1200, 300 + ((NUMBERS - 2) / 2) * 100));
  ...
  if (NUMBERS > 8) {
    ...
    background_middle_sprite.setTextureRect(
        sf::IntRect(0, 0, 1200, ((NUMBERS - 8) / 2) * 100));
  }
...
void SceneTableLead::draw() {
  ...
  background_top_sprite.setPosition(
      sf::Vector2f(background_top_rect.x, background_top_rect.y));
  if (NUMBERS > 8)
    background_middle_sprite.setPosition(
        sf::Vector2f(background_middle_rect.x, background_middle_rect.y));
  background_bottom_sprite.setPosition(
      sf::Vector2f(background_bottom_rect.x, background_bottom_rect.y));
  ...
  tablnohead_sprite.setPosition(
      sf::Vector2f(tablnohead_rect.x, tablnohead_rect.y));
  ...
  while (window->pollEvent(event)) {
    ...
    if (event.type == sf::Event::MouseWheelMoved &&
        event.mouseWheel.delta < 0 && background_bottom_rect.y > 600 - 300) {
