bool SceneInputLead::init(sf::RenderWindow &app) {
  ...
  window_input_color.r = 235;
  window_input_color.g = 235;
  window_input_color.b = 255;
...
char SceneInputLead::step() { return 0; }
...
void SceneInputLead::draw() {
  ...
  while (window->pollEvent(event)) {
    ...
    if (event.type == sf::Event::MouseButtonReleased) {
      if (!field_input_bl && sf::Mouse::getPosition(*window).x > 675 &&
          sf::Mouse::getPosition(*window).x < 825 &&
          sf::Mouse::getPosition(*window).y > 360 &&
          sf::Mouse::getPosition(*window).y < 410) {
        scene = scene_table_lead;
        return;
      }
      if (!field_input_bl && sf::Mouse::getPosition(*window).x > 375 &&
          sf::Mouse::getPosition(*window).x < 525 &&
          sf::Mouse::getPosition(*window).y > 360 &&
          sf::Mouse::getPosition(*window).y < 410) {
    ...
    if (event.type == sf::Event::KeyPressed) {
      ...
      if (field_input_bl && event.key.code == sf::Keyboard::BackSpace &&
          new_res.name.size() > 0)
        new_res.name.resize(new_res.name.size() - 1);
    }
    if (field_input_bl && event.type == sf::Event::TextEntered &&
        new_res.name.size() < 16) {
      if ((event.text.unicode >= 'A' && event.text.unicode <= 'Z') ||
          (event.text.unicode >= 'a' && event.text.unicode <= 'z'))
        new_res.name += event.text.unicode;
    }
  }
  if (sf::Mouse::getPosition(*window).x > 375 &&
      sf::Mouse::getPosition(*window).x < 525 &&
      sf::Mouse::getPosition(*window).y > 360 &&
      sf::Mouse::getPosition(*window).y < 410) {
  ...
  if (sf::Mouse::getPosition(*window).x > 675 &&
      sf::Mouse::getPosition(*window).x < 825 &&
      sf::Mouse::getPosition(*window).y > 360 &&
      sf::Mouse::getPosition(*window).y < 410) {
